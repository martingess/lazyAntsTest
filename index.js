...
async function startRecursive(arrayOfUsers, prevSum = 0) {
  
  if(!arrayOfUsers.length) return prevSum;
  const sliceOfUsersArray = arrayOfUsers.slice(0, 10);
  const arrayOfProcessingPromises = sliceOfUsersArray.map(user => processUser(user.id));
  const resultsOfPromises = await Promise.all(arrayOfProcessingPromises);
  const sum = prevSum + resultsOfPromises.reduce((sum, item) => sum + item);

  return startRecursive(arrayOfUsers.slice(10), sum);
